...
function DataGrid<R, SR, K extends Key>(
  ...
  const {
    ...
    rawColumns:
      ...
      rawColumns?.length > 0 &&
      !rawColumns.find(
        (item) => (item as Column<R, SR>).dataIndex === 'serialNoRow',
      )
  ...
  useEffect(() => {
    if (undoRef.current.length === 0 && columns.length > 0) {
      saveVersionHistory({
        rows: [...rows],
        columns: [...columns],
  ...
  function setCurrentData(step: UndoRedoProcessProps) {
    ...
    if (step?.styles) {
      if (setCellStyles && step?.styles?.cell)
        setCellStyles(step?.styles?.cell);
  ...
  function onRedo() {
    let X = redoRef.current[redoRef.current.length - 1];
    redoRef.current.pop();
  ...
  function saveVersionHistory(data: UndoRedoProcessProps) {
    undoRef.current.push({
      ...data,
      columns: data.columns.filter((item) => item.dataIndex !== 'serialNoRow'),
    });
    redoRef.current = [];
  }
  ...
  function handleKeyDown(event: KeyboardEvent<HTMLDivElement>) {
    ...
    if (
      ...
      if (keyCode === xKey) {
        appLog('handleCut clipboard', idx, rowIdx);

        handleGridCopy(handleGridCut);
        return;
      }
      if (keyCode === vKey) {
        appLog('handlePaste clipboard', idx, rowIdx);
    ...
    if (formulas) {
      ...
      if (isCorrectFormula(String(selectedCellValue))) {
        if (foundFormulaIndex >= 0) {
          ...
          let updatedTableFormula = [...formulas];
  ...
  const handleGridPaste = () => {
    ...
    if (selectedCellIsWithinViewportBounds) {
      navigator.clipboard
        ...
        .catch((error) => {
          appLog('Data copied to clipboard error', error);
        });
    }
  };
    ...
    if (selectedCellIsWithinViewportBounds) {
      ...
      navigator.clipboard
        ...
        .then(() => {
          ...
          appLog('Data copied to clipboard', data);
        })
        .catch((error) => {
          appLog('Data copied to clipboard error', error);
        });
  ...
  const selectSerialNoRow = useCallback((cellIdx?: number, rowIdx?: number) => {
      ...
      rowElements.push(
        renderRow(key, {
          ...
          setDraggedOverCellIdx:
            isDragging && !isDraggingLocked ? setDraggedOverCellIdx : undefined,
